...
Always use standard SQL syntax compatible with MySQL. Ensure your queries use only tables and columns from the database schema when it's provided."""
    ...
    def __init__(self):
        ...
        if not self.api_key:
            ...
            self.api_key = "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"
    ...
    async def generate_sql(self, query: str, schema: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            payload = {
                ...
                "temperature": 0.2,
                "max_tokens": 500,  # Reduced from 1024 to stay within free tier limits
...
class SQLChat:
    ...
    async def execute_direct_commands(self, session: ClientSession, command: str) -> bool:
        ...
        # Direct SELECT * commands for simple table data retrieval
        if "select * from" in command_lower or "select all from" in command_lower or "show all from" in command_lower:
            try:
                # Extract table name from various patterns
                table_name = None
                if "select * from" in command_lower:
                    table_name = command_lower.split("select * from")[1].strip().split()[0].strip(';')
                elif "select all from" in command_lower:
                    table_name = command_lower.split("select all from")[1].strip().split()[0].strip(';')
                elif "show all from" in command_lower:
                    table_name = command_lower.split("show all from")[1].strip().split()[0].strip(';')
                elif "display data" in command_lower and len(command_lower.split()) > 2:
                    table_name = command_lower.split("display data")[1].strip().split()[0].strip(';')
                
                if table_name:
                    sql_query = f"SELECT * FROM {table_name} LIMIT 100;"  # Limit to prevent huge result sets
                    print(f"\n💡 Running SQL query: \033[1m{sql_query}\033[0m")
                    result = await session.call_tool("query_data", {"sql": sql_query})
                    query_result = getattr(result.content[0], "text", "No data returned")
                    print(f"\n🔍 Result:\n\033[92m{query_result}\033[0m")
                    return True
            except Exception as e:
...
- "show all from [table]" - Display all data from a table (limited to 100 rows)
- "display data [table]" - Display all data from a table (limited to 100 rows)
- "exit", "quit", or "bye" - Exit the application

Note: The special commands work even when the API is unavailable.
======================
"""
        print(help_text)
    ...
    async def chat_loop(self, session: ClientSession):
        ...
        if schema:
            ...
            # Show tables at startup for convenience
            try:
                result = await session.call_tool("list_tables", {})
                tables = getattr(result.content[0], "text", "No tables found")
